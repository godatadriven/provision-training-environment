...
def add_firewall_rules():
    delegator.run(("gcloud compute firewall-rules delete allow-training-ports "))
    delegator.run(("gcloud compute firewall-rules create allow-training-ports "
                   "--allow tcp:9200,tcp:5601,tcp:5000 "
                   "--source-ranges 0.0.0.0/0 "
                   "--target-tags training-instance "
                   "--description 'Allow access to kibana, elastic and jupyter'"))

def init_host_file(hosts_path):
    with open(hosts_path, 'w') as f:
        f.write('[nlp-instances]')
        f.write('\n')
...
def get_instance_name_prefix(name, N_sequence=10):
    if name:
        ...
        return f"instance-{get_random_id(N_sequence)}"
